...
import { authMiddleware, expertMiddleware } from '../middleware/auth.js';
...
router.get('/:treatyId', async (req, res) => {
  try {
...
router.post('/', expertMiddleware, async (req, res) => {
  try {
...
router.put('/:treatyId', expertMiddleware, async (req, res) => {
  try {
...
router.delete('/:treatyId', expertMiddleware, async (req, res) => {
  try {
...
router.get('/in-force', async (req, res) => {
  try {
    const { limit = 20, skip = 0 } = req.query;
